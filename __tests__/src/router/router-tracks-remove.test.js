...
const {settings, settings2} = require('../../data/request');
const mod = require('../../../src/router/router-tracks-remove');
...
describe('Router Tracks Remove', () => {
  ...
  const triggerId = '1276529178274.879979449463.29d062b39a59e254577893dbac2d6556';
  ...
  describe('Handler', () => {
    ...
    it('should return a public ack', async () => {
      ...
      mockCheckSettings.checkIsSetup.mockResolvedValue(settings2);
      ...
      expect(mockSns.publish).toHaveBeenCalledWith({
        ...
        TopicArn: process.env.SNS_PREFIX + 'tracks-remove-open',
      });
      expect(mockSlackReply.publicAck).toHaveBeenCalledWith('');
      expect(mockSlackModal.openModal).toHaveBeenCalledWith(teamId, channelId, triggerId, mockConfig.slack.actions.empty_modal, 'Remove Tracks', null, 'Cancel');
    });
    ...
    it('should return an empty response', async () => {
      ...
      expect(await mod.handler(event[0])).toStrictEqual({statusCode: 200, body: ''});
      expect(mockCheckSettings.checkIsSetup).toHaveBeenCalledWith(teamId, channelId);
      expect(mockSns.publish).toHaveBeenCalledWith({
        Message: JSON.stringify({
          teamId,
